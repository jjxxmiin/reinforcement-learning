...
# PIL 패키지 설치를 위해서 >> pip install image 실행했음.

PhotoImage = ImageTk.PhotoImage
UNIT = 100  # 픽셀 수
HEIGHT = 5  # 그리드월드 세로
WIDTH = 5  # 그리드월드 가로
TRANSITION_PROB = 1  # 상태 전이 확률은 1로 고정
...
class GraphicDisplay(tk.Tk):
    ...
    def _build_canvas(self):
        ...
        policy_button = Button(self, text="Move", 
...
class Env:
    def __init__(self):
        self.transition_probability = TRANSITION_PROB  # 상태 전이 확률
        self.width = WIDTH  # Grid World 의 폭
        self.height = HEIGHT  # Grid World 의 높이
        self.reward = [[0] * WIDTH for _ in range(HEIGHT)]  # 상태별 보상 초기화
        self.possible_actions = POSSIBLE_ACTIONS  # 상태별 선택 가능한 행동
        self.reward[2][2] = 1  # (2,2) 좌표 동그라미 위치(=goal)에 보상 +1
        self.reward[1][2] = -1  # (1,2) 좌표 세모 위치(= 함정)에 보상 -1
        self.reward[2][1] = -1  # (2,1) 좌표 세모 위치(= 함정)에 보상 -1
        self.all_state = []

        # 모든 상태(=모든 좌표값)를 all_state 에 저장
